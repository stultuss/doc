...
RESTful API 是目前比较成熟的一套互联网程序的的 API 设计理论，并且被大多数的互联网开发人员所掌握。
...
> 由于 RESTful API 是表示表述性无状态传输，无需 session，所以每次请求都得带上身份认证信息，所以它的安全特性都需我们自己实现
...
比较通用的解决方案是：Facebook 的 Graph API，但没有开源。
...
- JWT
  ...
  > 将签名，数据，认证包装成一个紧凑的 JSON 对象进行传输，适合放在 HTTP Authorization 头中

- AccessToken

  > 经过用户身份认证后，服务端分配给客户端的一个 token，这个 token 会有一个过期时间，客户端每次请求 API 时候需要携带一个签名（签名由客户端 KEY + 请求参数 + token 合并计算生成）

### 数据篡改

签名参数中包含请求中重要参数的计算，改动参数，签名值都会不同，因此无法篡改。

### 重放攻击

签名参数中包含客户端时间戳，服务端验证 sign 中的时间戳来对请求进行过滤。
